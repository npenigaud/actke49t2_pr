...
INTEGER(KIND=JPIM), INTENT (IN)::LDCONV (KLON, KLEV)
...
temp (REAL (KIND=JPRB), ZRTV, (:, :))

temp (REAL (KIND=JPRB), ZZ, (:, :))
temp (REAL (KIND=JPRB), ZGDZF, (:, :))
temp (REAL (KIND=JPRB), ZTHETA, (:, :))

temp (REAL (KIND=JPRB), ZLOCPEXF, (:, :))
temp (REAL (KIND=JPRB), ZTHETALF, (:, :))
temp (REAL (KIND=JPRB), ZTHETAVL, (:, :))

temp (REAL (KIND=JPRB), ZECTF, (:, :))
temp (REAL (KIND=JPRB), ZLMECTF, (:, :))
temp (REAL (KIND=JPRB), ZGKTF, (:, :))

temp (REAL (KIND=JPRB), ZGKUH, (:, :))
temp (REAL (KIND=JPRB), ZGKTH, (:, :))
temp (REAL (KIND=JPRB), ZLM, (:, :))

REAL (KIND=JPRB)::ZWSTAR 
REAL (KIND=JPRB)::ZUSTAR 
REAL (KIND=JPRB)::ZECTINT 
REAL (KIND=JPRB)::ZQCINT 
temp (INTEGER (KIND=JPIM), ICM, (:, :))
...
temp (REAL (KIND=JPRB), ZAH, (:, :))
temp (REAL (KIND=JPRB), ZQSLTLH, (:, :))
...
alloc (ZRTV, 1,1, KLON,KLEV)
alloc (ZZ, 1,1, KLON,KLEV)
alloc (ZGDZF, 1,1, KLON,KLEV)
alloc (ZTHETA, 1,1, KLON,KLEV)
alloc (ZLOCPEXF, 1,1, KLON,KLEV)
alloc (ZTHETALF, 1,1, KLON,KLEV)
alloc (ZTHETAVL, 1,1, KLON,KLEV)
alloc (ZECTF, 1,1, KLON,KLEV)
alloc (ZLMECTF, 1,1, KLON,KLEV)
alloc (ZGKTF, 1,1, KLON,KLEV)
alloc (ZGKUH, 1,0, KLON,KLEV)
alloc (ZGKTH, 1,0, KLON,KLEV)
alloc (ZLM, 1,0, KLON,KLEV)
alloc (ICM, 1,1, KLON,KLEV)
alloc (ZAH, 1,1, KLON,KLEV)
alloc (ZQSLTLH, 1,1, KLON,KLEV)
...
!  tab1(jlon,jlev)=zrtv(jlon,jlev)
ENDDO
!!!!!!!!!!do jlev=ktdian, klev
!  tab2(jlon,jlev)=zrtv(jlon,jlev)
!!!!!!!!!!enddo
DO JLEV=KTDIAN+1, KLEV
  
  ZGDZF (JLON, JLEV)=PAPHIF (JLON, JLEV-1)-PAPHIF (JLON, JLEV)
!  tab3(jlon,jlev)=zrtv(jlon,jlev)
ENDDO
!tab4(jlon,:)=zrtv(jlon,:)
