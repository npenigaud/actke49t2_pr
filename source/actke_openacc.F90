SUBROUTINE ACTKE_OPENACC (YDCST, YDLDDH, YDMDDH, YDML_PHY_MF, KIDIA, KFDIA, KLON, KTDIAT, KTDIAN, KLEV&
&, PAPHI, PAPHIF, PAPRS, PAPRSF, PDELP, PR, PT, PU, PV, PQ, PQICONV, PQLCONV, PLSCPE, PCD, PCH, PGZ0&
&, PTS, PQS, PQICE, PQLI, PECT, PPRODTH, PNLAB, PNLABCVP, PKTROV, PKQROV, PKQLROV, PKUROV, PXTROV, PXUROV&
&, PNBVNO, PNEBS, PQCS, PNEBS0, PQCS0, PCOEFN, PFECT, PFECTI, PECT1, PTPRDY, PEDR, YDSTACK)
!$omp declare target
...
temp (REAL (KIND=JPRB), ZPHI3, (:, :))
temp (REAL (KIND=JPRB), ZLMECT, (:, :))
temp (REAL (KIND=JPRB), ZUSLE, (:, :))

temp (REAL (KIND=JPRB), ZPRDY, (:, :))
temp (REAL (KIND=JPRB), ZPRODTH, (:, :))
temp (REAL (KIND=JPRB), ZDIAG, (:, :))

temp (REAL (KIND=JPRB), ZDISS, (:, :))
temp (REAL (KIND=JPRB), ZDIFF, (:, :))

temp (REAL (KIND=JPRB), ZECT1, (:, :))
temp (REAL (KIND=JPRB), ZECT, (:, :))
temp (REAL (KIND=JPRB), ZDELPSG, (:, :))
temp (REAL (KIND=JPRB), ZDET, (:, :))

temp (REAL (KIND=JPRB), ZECTCLS, (:))
temp (REAL (KIND=JPRB), ZKCLS, (:))

temp (REAL (KIND=JPRB), ZDIFFAR, (:, :))
temp (REAL (KIND=JPRB), ZTABFL, (:, :))
temp (REAL (KIND=JPRB), ZTABHL, (:, :))

temp (REAL (KIND=JPRB), ZFCORTKE, (:, :))
temp (REAL (KIND=JPRB), ZFDIFF, (:, :))
temp (REAL (KIND=JPRB), ZFDISS, (:, :))
temp (REAL (KIND=JPRB), ZFPRDY, (:, :))
temp (REAL (KIND=JPRB), ZFPRTH, (:, :))

temp (REAL (KIND=JPRB), ZEDR, (:, :))
temp (REAL (KIND=JPRB), ZTCORTKE, (:, :))
temp (REAL (KIND=JPRB), ZTDIFF, (:, :))
temp (REAL (KIND=JPRB), ZTDISS, (:, :))
temp (REAL (KIND=JPRB), ZTPRTH, (:, :))
...
alloc2d (ZPHI3, 1,1, KLON,KLEV)
alloc2d (ZLMECT, 1,1, KLON,KLEV)
alloc2d (ZUSLE, 1,1, KLON,KLEV)
alloc2d (ZPRDY, 1,1, KLON,KLEV)
alloc2d (ZPRODTH, 1,1, KLON,KLEV)
alloc2d (ZDIAG, 1,1, KLON,KLEV)
alloc2d (ZDISS, 1,1, KLON,KLEV)
alloc2d (ZDIFF, 1,1, KLON,KLEV)
alloc2d (ZECT1, 1,1, KLON,KLEV)
alloc2d (ZECT, 1,1, KLON,KLEV)
alloc2d (ZDELPSG, 1,1, KLON,KLEV)
alloc2d (ZDET, 1,1, KLON,KLEV)
alloc (ZECTCLS, 1, KLON)
alloc (ZKCLS, 1, KLON)
alloc2d (ZDIFFAR, 1,1, KLON,KLEV)
alloc2d (ZTABFL, 1,1, KLON,KLEV)
alloc2d (ZTABHL, 1,0, KLON,KLEV)
alloc2d (ZFCORTKE, 1,0, KLON,KLEV)
alloc2d (ZFDIFF, 1,0, KLON,KLEV)
alloc2d (ZFDISS, 1,0, KLON,KLEV)
alloc2d (ZFPRDY, 1,0, KLON,KLEV)
alloc2d (ZFPRTH, 1,0, KLON,KLEV)
alloc2d (ZEDR, 1,1, KLON,KLEV)
alloc2d (ZTCORTKE, 1,1, KLON,KLEV)
alloc2d (ZTDIFF, 1,1, KLON,KLEV)
alloc2d (ZTDISS, 1,1, KLON,KLEV)
alloc2d (ZTPRTH, 1,1, KLON,KLEV)
alloc2d (LLCONV, 1,1, KLON,KLEV)
